...
// Vendored from React Bits (DomeGallery-TS-TW), with local changes marked
// `// local:` — client directive, a CSSProperties import, the restored
// scroll-lock rule, and per-photo event metadata shown when a tile opens.
import { useEffect, useMemo, useRef, useCallback, type CSSProperties } from 'react';
import { useGesture } from '@use-gesture/react';

// local: extended with the event a photo belongs to, so opening a tile can
// name it rather than showing a bare image.
// local: lets a style object carry CSS custom properties without casting each
// one to any. Real CSS properties stay type-checked; only `--*` keys are extra.
...
export default function DomeGallery({
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        // local: PointerEvent.pointerType is a plain string; narrow it to the
        // three values this component tracks instead of casting through any.
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        if (last) {
          ...
          // local: eslint's purity rule cannot tell that this runs in a pointer
          // callback rather than during render. Reading the clock when a drag
          // ends is the point — it is what suppresses the click that follows.
  ...
  const cssStyles = `
    ...
    /* local: restored. Shipped commented out with double slashes, which CSS
       does not treat as a comment, so the rule was dead: lockScroll() added a
       class that styled nothing and the page scrolled behind the gallery on
       touch drag. Backticks are avoided in here on purpose — this block is a
    ...
    /* local: caption for the opened photo. Sits over the bottom of the image
       with its own scrim so it stays readable on a light photo, and is excluded
       from the grayscale filter applied to the picture itself. */
    /* local: until the real photos land, every tile is a broken image, and a
       sphere of rendered alt text reads as a fault rather than an empty state.
       The alt attribute stays on the element for assistive tech; this only
       stops the browser painting it as visible text. */
    .item__image img {
