...
// Vendored from React Bits (ParticleText-TS-TW). Local changes marked `// local:`.
//
// local: upstream already respects prefers-reduced-motion for the particles'
// positions — it snaps them and disables drift and repel — but the render loop
// re-schedules itself unconditionally, so it keeps redrawing an identical frame
// for the life of the page. It also has no off-screen pause. Both are fixed
// below; the identity spec requires them.
//
// Colours must be real hex: hexToRgb below rejects anything else and the
// component falls back, so a caller passes resolved values, not var(--token).
...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    let gathering = false;
    let gatherStart = 0;
    // local: set by the IntersectionObserver below.
    let paused = false;
    ...
    const render = (now: number): void => {
      ...
      // local: upstream re-schedules here unconditionally. Once the gather ends
      // under reduced motion the drift, the repel and the easing are all off,
      // so every subsequent frame draws exactly the same image — forever, and
      // off-screen too. Stop when there is nothing left to move.
      if (paused || (reducedMotion && !gathering)) {
        animationFrame = null;
        return;
      }
    ...
    // local: without this the wordmark keeps animating while the reader is
    // anywhere else on the page.
    const visibilityObserver = new IntersectionObserver(
      entries => {
        paused = !entries[0].isIntersecting;
        if (!paused) ensureRenderLoop();
      },
      { threshold: 0 }
    );
