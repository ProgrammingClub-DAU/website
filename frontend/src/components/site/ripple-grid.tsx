...
// Vendored from React Bits (RippleGrid-TS-TW). Local changes marked `// local:`.
//
// local: upstream's loop never stops — no reduced-motion check, no off-screen
// pause — and it assumes WebGL is available. Both guards are the identity
// spec's requirements; the availability check keeps a decoration from taking
// the page it sits behind to an error boundary. It does release its context on
// unmount, which Particles did not.
//
// gridColor must be a real colour: it is parsed to a vec3 below, with no CSS
// cascade to resolve a var(--token) against, so the caller resolves it.
...
const RippleGrid: React.FC<Props> = ({
  ...
  // local: was `any`. The second effect below writes `.value` on each of
  // these, which is all this ref is for.
  const uniformsRef = useRef<Record<string, { value: unknown }> | null>(null);

  useEffect(() => {
    if (!containerRef.current) return;
    // local: captured once. Reading containerRef.current in the cleanup would
    // read whatever the ref points at then, which is not necessarily the node
    // these listeners were attached to.
    ...
    // local: where WebGL is unavailable the constructor throws, and an
    // unhandled throw in a client component takes the whole page with it.
...
void main() {
    ...
    // local: split from the scheduling below, so reduced motion can draw one
    // still frame instead of leaving an empty rectangle.
    ...
    // without an observer this keeps a GPU context busy off-screen.
    ...
    // The second effect below pushes every visual prop into the uniforms live,
    // so re-running this one would tear down the WebGL context for nothing.
