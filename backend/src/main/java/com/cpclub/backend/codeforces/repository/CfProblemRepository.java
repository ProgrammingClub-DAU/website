...
public interface CfProblemRepository extends JpaRepository<CfProblem, Long> {
    ...
    @Query(value = "SELECT cp.* FROM cf_problems cp WHERE cp.rating >= :minRating AND cp.rating <= :maxRating AND NOT EXISTS (SELECT 1 FROM cf_problem_tags t WHERE t.problem_id = cp.id AND t.tag = '*special') ORDER BY RANDOM() LIMIT :limit", nativeQuery = true)
